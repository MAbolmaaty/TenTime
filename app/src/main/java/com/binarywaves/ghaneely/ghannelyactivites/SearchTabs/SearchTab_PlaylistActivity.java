...
public class SearchTab_PlaylistActivity extends Fragment {
    ...
    private ArrayList<PlaylistnotificationResponse> mAlbumList;
    private String like;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        bundle = getArguments();
        if (bundle != null) {
            mAlbumList = bundle.getParcelableArrayList("playlistlst");
    ...
    private void InitlizeUi() {
        ...
        lvtrack.setOnItemClickListener((parent, view, position, id) -> {
            ...
            if (Constants.isNetworkOnline(context)) {
                ...
                AddUserSearchResult(mAlbumList.get(position).getPlaylistId(), Integer.valueOf("4"), UserSearchID);
                Intent playTrack1 = new Intent(context, PlayListTracksActivity.class);
                playTrack1.putExtra("id", mAlbumList.get(position).getPlaylistId());
        ...
        if (mAlbumList != null && mAlbumList.size() > 0) {
            MyPlaylistAdaptor mAdaptor = new MyPlaylistAdaptor(context, R.layout.myplaylist_item_list, null, mAlbumList);
            lvtrack.setAdapter(mAdaptor);
