...
public class FriendTab_followArtActivity extends Fragment {
    ...
    private static AbumGallareyAdaptor myTrackadAdaptor;
 ...
 //       bundle = getArguments();
        if (AppSingleTon.getInstance().getFolloweredSingers() != null)
            singerObjects =AppSingleTon.getInstance().getFolloweredSingers() ;
        Log.e("bundle", singerObjects + "");
    ...
    private void InitlizeUi() {
        FriendsTabActivity.selectedfragment=2;
        ...
        if (language.equalsIgnoreCase("ar")) {
            ...
            if (Constants.isNetworkOnline(context)) {
                if (position >= singerObjects.size()) {
                    position = position % singerObjects.size();
                }
                Intent playTrack = new Intent(context, ArtistTabsActivity.class);
                playTrack.putExtra("playlistId", singerObjects.get(position).getSingerId());
        ...
        changeUI1(context);
        if (singerObjects != null && singerObjects.size() > 0)
            SetOtherAlbumLst(singerObjects);
    ...
    private void SetOtherAlbumLst(ArrayList<ArtistRadio> mtrackList1) {
        ArtistRadioAdaptor artist_radio_adaptor = new ArtistRadioAdaptor(context,
                R.layout.other_relatedalbumlistitem, singerObjects);
        lvtrack.setAdapter(artist_radio_adaptor);
