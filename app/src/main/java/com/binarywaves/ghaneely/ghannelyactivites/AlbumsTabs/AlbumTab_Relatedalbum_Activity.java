...
public class AlbumTab_Relatedalbum_Activity extends Fragment {
    ...
    private ArrayList<SlideAlbumObject> relatedalbumLst;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        bundle = getArguments();
        if (bundle != null)
            relatedalbumLst = bundle.getParcelableArrayList("relatedalbumLst");
        Log.e("bundle", relatedalbumLst + "");
    ...
    private void InitlizeUi() {
        AlbumTabsActivity.selectedfragment=2;
        ...
        lvtrack.setOnItemClickListener((parent, view, position, id) -> {
            ...
            if (Constants.isNetworkOnline(context)) {
                if (position >= relatedalbumLst.size()) {
                    position = position % relatedalbumLst.size();
                }
                // StopService();
                Intent albumIntent = new Intent(context, AlbumTabsActivity.class);
                albumIntent.putExtra("album", relatedalbumLst.get(position));
        ...
        if (relatedalbumLst != null && relatedalbumLst.size() > 0)
            SetRelatedAlbumLst(relatedalbumLst);
    ...
    private void SetRelatedAlbumLst(ArrayList<SlideAlbumObject> mtrackList1) {
