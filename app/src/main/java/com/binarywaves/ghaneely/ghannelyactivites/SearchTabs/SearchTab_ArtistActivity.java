...
public class SearchTab_ArtistActivity extends Fragment {
    ...
    private ArrayList<ArtistRadio> mAlbumList;
    private String like;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        bundle = getArguments();
        if (bundle != null) {
            mAlbumList = bundle.getParcelableArrayList("singerLst");
    ...
    private void InitlizeUi() {
        ...
        lvtrack.setOnItemClickListener((parent, view, position, id) -> {
            ...
            if (Constants.isNetworkOnline(context)) {
                ...
                AddUserSearchResult(String.valueOf(mAlbumList.get(position).getSingerId()), Integer.valueOf("1"), UserSearchID);
                Intent playTrack = new Intent(context, ArtistTabsActivity.class);
                playTrack.putExtra("playlistId", mAlbumList.get(position).getSingerId());
        ...
        if (mAlbumList != null && mAlbumList.size() > 0) {
            ArtistRadioAdaptor artist_radio_adaptor = new ArtistRadioAdaptor(context,
                    R.layout.other_relatedalbumlistitem, mAlbumList);
