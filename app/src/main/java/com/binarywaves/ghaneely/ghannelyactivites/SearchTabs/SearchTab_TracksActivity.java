...
public class SearchTab_TracksActivity extends Fragment {
    ...
    private ArrayList<TrackLst> mtracklist;
    private String like;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        activityView = inflater.inflate(R.layout.activity_searchtracks, container, false);
        context = getContext();
        language = LanguageHelper.getCurrentLanguage(context);

        bundle = getArguments();
        if (bundle != null) {
            mtracklist = bundle.getParcelableArrayList("trackLst");
            UserSearchID = bundle.getString("UsersearchID");

            Log.e("bundle", mtracklist + "");
        }
    ...
    private void InitlizeUi() {
        ...
        lvtrack .setOnItemClickListener((parent, view, position, id) -> {
            ...
            if (Constants.isNetworkOnline(context)) {
                ...
                favo = newtrackobject.get(position).getIsFavourite();

                if (newtrackobject.get(position).getIsFavourite().equalsIgnoreCase("true")) {
                    // change icon
                ...
                AddUserSearchResult(newtrackobject.get(position).getTrackId(), Integer.valueOf("3"), UserSearchID);
        ...
        if (mtracklist != null && mtracklist.size() > 0) {
            for (int i = 0; i < mtracklist.size(); i++) {
                ...
                mTrack.setSingerId(mtracklist.get(i).getSingerId());
                mTrack.setAlbumId(mtracklist.get(i).getAlbumId());

                mTrack.setTrackId(mtracklist.get(i).getTrackId());
                mTrack.setTrackEnName(mtracklist.get(i).getTrackEnName());
                mTrack.setAlbumEnName(mtracklist.get(i).getAlbumEnName());
                mTrack.setSingerEnName(mtracklist.get(i).getSingerEnName());
                mTrack.setTrackArName(mtracklist.get(i).getTrackArName());
                mTrack.setAlbumArName(mtracklist.get(i).getAlbumArName());
                mTrack.setSingerArName(mtracklist.get(i).getSingerArName());

                mTrack.setTrackImage(mtracklist.get(i).getTrackImage());
                mTrack.setTrackPath(mtracklist.get(i).getTrackPath());
                mTrack.setIsFavourite(mtracklist.get(i).getIsFavourite());
                mTrack.setIsRBT(mtracklist.get(i).getIsRBT());
                mTrack.setLikesCount(mtracklist.get(i).getLikesCount());
                mTrack.setListenCount(mtracklist.get(i).getListenCount());
                mTrack.setTrackLength(mtracklist.get(i).getTrackLength());
                mTrack.setHasLyrics(mtracklist.get(i).getHasLyrics());

                mTrack.setLyricFile(mtracklist.get(i).getLyricFile());
                mTrack.setVideoID(mtracklist.get(i).getVideoID());
                mTrack.setIsPremium(mtracklist.get(i).getIsPremium());
                mTrack.setIsDownloaded(mtracklist.get(i).getIsDownloaded());
