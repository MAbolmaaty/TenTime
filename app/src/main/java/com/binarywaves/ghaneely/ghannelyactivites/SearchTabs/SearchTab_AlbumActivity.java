...
public class SearchTab_AlbumActivity extends Fragment {
    ...
    private ArrayList<AlbumLst> mAlbumList;
    private String like;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        bundle = getArguments();
        if (bundle != null) {
            mAlbumList = bundle.getParcelableArrayList("albumLst");
        ...
        if (mAlbumList != null && mAlbumList.size() > 0) {
            ...
            for (int j = 0; j < mAlbumList.size(); j++) {
                SlideAlbumObject mPlayist = new SlideAlbumObject();

                mPlayist.setAlbumId(mAlbumList.get(j).getAlbumId());
                mPlayist.setAlbumEnName(mAlbumList.get(j).getAlbumEnName());
                mPlayist.setAlbumArName(mAlbumList.get(j).getAlbumArName());
                mPlayist.setSingerEnName(mAlbumList.get(j).getSingerArName());
                mPlayist.setSingerArName(mAlbumList.get(j).getSingerEnName());
                mPlayist.setAlbumImgPath(mAlbumList.get(j).getAlbumImgPath());
    ...
    private void InitlizeUi() {
        ...
        lvtrack.setOnItemClickListener((parent, view, position, id) -> {
            ...
            if (Constants.isNetworkOnline(context)) {
                ...
                AddUserSearchResult(String.valueOf(mPlaylistObjects.get(position).getAlbumId()), Integer.valueOf("2"), UserSearchID);
        ...
        if (mPlaylistObjects != null && mPlaylistObjects.size() > 0) {
            GalleryAdaptor songAdaptor = new GalleryAdaptor(context, R.layout.other_relatedalbumlistitem, mPlaylistObjects, 2);
